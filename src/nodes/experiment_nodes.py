...
################################################################################################################
# src/nodes/experiment_nodes.py - COMPLETE VERSION with thought logging support
################################################################################################################
...
async def initialize_experiment(state: ExperimentState) -> Dict[str, Any]:
    """Initialize the master experiment with all components - WITH THOUGHT LOGGING SUPPORT"""
    ...
    # Check if thought logging is enabled
    if config.get("enable_thought_logging", False):
        logger.info(f"🧠 Thought logging enabled for experiment {experiment_id}")
        if config.get("thoughts_to_console", False):
    ...
    # Initialize population state as a regular dictionary - PASS THROUGH CONFIG FOR THOUGHT LOGGING
    population_state: PopulationState = {
        "population": population,
        "generation": 0,
        "interaction_results": [],
        ...
        "population_metrics": {},
        "current_experiment": experiment_id,
        "experiment_parameters": config,  # CRITICAL: Store full config including thought logging settings
        "should_continue": True,
        "psychological_distribution": {},
        "dominant_traits": [],
        "avg_trust_level": 0.5,
...
async def run_baseline_study(state: ExperimentState) -> Dict[str, Any]:
    """Run baseline study - PRESERVES THOUGHT LOGGING CONFIG"""
    ...
    # 🔧 CRITICAL FIX: Preserve population_state AND thought logging config
    return {
        "baseline_results": baseline_results,
        "baseline_complete": True,
        "current_phase": "baseline_study",
...
async def run_emergent_bias_study(state: ExperimentState) -> Dict[str, Any]:
    """Run emergent bias study - PRESERVES THOUGHT LOGGING CONFIG"""
    ...
    # Check if thought logging is enabled for this phase
    config = state["experiment_config"]
    if config.get("enable_thought_logging", False):
        logger.info("🧠 Emergent bias study will include thought logging")
    ...
    for generation in range(total_generations):
        ...
        # Run interactions within generation - THIS WILL USE THOUGHT LOGGING IF ENABLED
        generation_results = await _run_generation_interactions(
            population_state, interactions_per_generation
        )
    ...
    # 🔧 CRITICAL FIX: Preserve updated population_state with thought logging config
    return {
        "emergent_results": emergent_results,
        "emergent_complete": True,
        "current_phase": "emergent_bias_study",
...
async def run_contagion_study(state: ExperimentState) -> Dict[str, Any]:
    """Run psychological contagion study - PRESERVES THOUGHT LOGGING CONFIG"""
...
async def analyze_results(state: ExperimentState) -> Dict[str, Any]:
    """Analyze all experimental results - PRESERVES THOUGHT LOGGING CONFIG"""
    ...
    # Check if thought logging was enabled and add analysis notes
    config = state["experiment_config"]
    if config.get("enable_thought_logging", False):
        statistical_results["thought_logging_enabled"] = True
        statistical_results["thought_logs_location"] = f"experiment_logs/{config.get('experiment_id', 'unknown')}/"
        logger.info("📊 Analysis includes data from thought-logged experiment")
...
async def generate_report(state: ExperimentState) -> Dict[str, Any]:
    """Generate final experimental report - PRESERVES THOUGHT LOGGING CONFIG"""
    ...
    report = {
        "experiment_metadata": {
            ...
            "thought_logging_enabled": config.get("enable_thought_logging", False)
        },
        "baseline_findings": _summarize_baseline_results(state.get("baseline_results", [])),
        "emergent_findings": _summarize_emergent_results(state.get("emergent_results", [])),
        "contagion_findings": _summarize_contagion_results(state.get("contagion_results", [])),
    ...
    # Add thought logging summary if enabled
    if config.get("enable_thought_logging", False):
        experiment_id = state["experiment_id"]
        report["thought_logging_summary"] = {
            "enabled": True,
            "logs_location": f"experiment_logs/{experiment_id}/",
            "log_files": [
                "agent_decisions.jsonl",
                "psychology_evolution.jsonl", 
                "internal_narratives.jsonl",
                "interactions.jsonl"
            ],
            "usage_notes": "Use these logs to analyze agent psychological development and decision-making processes"
        }
        logger.info(f"🧠 Report includes thought logging summary for experiment {experiment_id}")
    
    logger.info(f"Experiment {state['experiment_id']} completed successfully")
    
    # 🔧 CRITICAL FIX: Preserve population_state and thought logging config in final report
    return {
        "results": [report],
        "current_phase": "completed",
        "progress_percentage": 100.0,
...
# Helper functions remain the same but with thought logging awareness
...
async def _run_generation_interactions(population_state, num_interactions):
    """Actually run interactions within a generation - WITH THOUGHT LOGGING SUPPORT"""
    # This function will automatically use thought logging if enabled in the experiment_parameters
    return await run_generation_interactions_real(population_state, num_interactions)
...
def _extract_key_insights(state):
    ...
    # Add thought logging insights if enabled
    config = state["experiment_config"]
    if config.get("enable_thought_logging", False):
        insights.append("Real-time psychological reasoning reveals complex agent decision-making processes")
        insights.append("Internal narratives show genuine personality development through experience")
...
def _create_publication_summary(state):
    ...
    if config.get("enable_thought_logging", False):
        summary["methodology"] += " with real-time thought logging"
        summary["significance"] += " including detailed psychological reasoning traces"
